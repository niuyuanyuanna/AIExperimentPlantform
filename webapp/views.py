        ...
        if CheckForm.is_valid():
            ...
            if user is not None:
                if user.is_active:
                    ...
                    history_login.save()
                    userinfo=Suser.objects.filter(id=user.id).values('is_superuser',"is_active","is_staff")[0]
                    request.session['is_superuser'] = userinfo["is_superuser"]  
                    request.session['is_active'] = userinfo["is_active"]  
                    request.session['is_staff'] = userinfo["is_staff"] 
                    request.session['web_domain'] = django_settings.WEB_DOMAIN
...
def Logout(request):
    ...
    if request.session.get('user_id'):
        try:
            containerinfo=DockerContainer.objects.filter(status='running').values("image","Name",'hostip')
            DockerContainerCount2 = DockerContainer.objects.filter(status='running')
            print ("Docker_running="),DockerContainerCount2
            for it in containerinfo:
                hostip = it['hostip']
                dockerinfo=DockerHost.objects.filter(hostip=hostip).values("hostip","port")
                docker_port = dockerinfo[0]['port']
                Container_name = it['Name']
                image = it['image']
                if Container_name == image + '-'+ str(request.session['user_id']):
